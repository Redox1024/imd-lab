...
main.addEventListener('input',event=>{const input=event.target,key=input.dataset.field;if(!key)return;const record=currentRecords()[recordIndex];record[key]=input.type==='checkbox'?input.checked:input.type==='number'?(input.value===''?'':Number(input.value)):input.value;refreshRecordControls(record);markDirty();queueLive();});
...
main.addEventListener('click',async event=>{
 ...
 if(action==='add'){const item=newRecord(section);records.push(item);recordIndex=currentRecords().indexOf(item);fieldPage=0;render();main.scrollTop=0;markDirty();report(managedLists.includes(active)?'Entry added. Fill in the fields below, then choose Preview / Download JSON.':'Entry added. Use Previous / Next to review all fields.');}
