...
import {upsertImported,doiValue} from './publication-data.js';
import {lookupDOI,normalizeDOI,mergeDOIFields} from './doi-import.js';
import {orderRecords} from './list-order.js';
import {authorEntries,normalizeAuthorRoles,updateAuthorRole,renderAuthors,authorLegend} from './author-roles.js';
...
function fieldMarkup(spec,record){
 ...
 if(active==='publications'&&key==='authors')input+='<small>Enter full names separated by commas, or put one author on each line. Select roles and bold text below; multiple authors can share a role.</small><div id="author-role-controls">'+authorRoleControls(record)+'</div>';
...
function authorRoleControls(record){
 ...
 const options=[['first','First / co-first †'],['corresponding','Corresponding *'],['bold','Bold']];
