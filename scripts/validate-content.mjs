...
for(const file of ['index.html','editor.html','styles.css','schema.js','app.js','hero-carousel.js','editor.js','pagination.js','live-preview.js','publication-data.js','document-import.js','doi-import.js','list-order.js','vendor/pdf.mjs','vendor/pdf.worker.mjs','vendor/tesseract.min.js','vendor/tesseract.worker.min.js','data/journal-if-template.csv','favicon.svg'])await access(new URL(file,base));
...
console.log('Content and required assets verified:',Object.fromEntries(Object.entries(data).filter(([,value])=>Array.isArray(value)).map(([key,value])=>[key,value.length])));
